...
export default function Debt() {
  ...
  const inputCls = (f: string) =>
    `w-full bg-[#0E1117] border ${errors[f] ? 'border-[#EF4444]' : 'border-on-surface/10'} rounded-xl px-3 py-2.5 text-sm text-on-surface placeholder:text-slate-600 focus:outline-none focus:border-[#2962FF]`;
    ...
    <div className="space-y-8">
      {/* Header */}
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-bold font-headline text-on-surface tracking-tight">Debt Overview</h2>
            <p className="text-sm text-on-surface/40 font-body mt-0.5">Track your loans and EMIs.</p>
          </div>
          <button
        ...
        <div className="bg-on-surface/5 backdrop-blur-xl border border-on-surface/10 shadow-[0_4px_20px_rgba(0,0,0,0.2)] rounded-xl p-5">
          <h3 className="font-bold font-headline text-on-surface tracking-tight mb-3">Monthly Income (for DTI ratio)</h3>
          <div className="flex gap-3">
            <input
              ...
              className="flex-1 bg-[#0E1117] border border-on-surface/10 rounded-xl px-3 py-2.5 text-sm text-on-surface placeholder:text-slate-600 focus:outline-none focus:border-[#2962FF]"
            />
            <button
              onClick={handleSaveIncome}
              className="px-4 py-2.5 bg-[#2A2F38] hover:bg-[#3a4a5f] text-on-surface/80 text-sm font-semibold rounded-xl border border-[#3a4a5f] transition-colors"
            >
              Save
            </button>
          </div>
          {monthlyIncome > 0 && <p className="text-xs text-on-surface/40 font-body mt-2">Monthly income: {inr(monthlyIncome)}</p>}
        </div>
        ...
        {showForm && (
          <div className="bg-on-surface/5 backdrop-blur-xl border border-on-surface/10 shadow-[0_4px_20px_rgba(0,0,0,0.2)] rounded-2xl p-6">
            <h3 className="font-bold font-headline text-on-surface tracking-tight mb-5">{editId ? 'Edit Loan' : 'Add New Loan'}</h3>
            <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="sm:col-span-2">
                <label className="block text-xs text-on-surface/40 font-body uppercase tracking-wider mb-1.5">Loan Name</label>
              ...
              <div>
                <label className="block text-xs text-on-surface/40 font-body uppercase tracking-wider mb-1.5">Loan Type</label>
              ...
              <div>
                <label className="block text-xs text-on-surface/40 font-body uppercase tracking-wider mb-1.5">Bank / Lender</label>
              ...
              <div>
                <label className="block text-xs text-on-surface/40 font-body uppercase tracking-wider mb-1.5">Principal (₹)</label>
              ...
              <div>
                <label className="block text-xs text-on-surface/40 font-body uppercase tracking-wider mb-1.5">Outstanding (₹)</label>
              ...
              <div>
                <label className="block text-xs text-on-surface/40 font-body uppercase tracking-wider mb-1.5">Monthly EMI (₹)</label>
              ...
              <div>
                <label className="block text-xs text-on-surface/40 font-body uppercase tracking-wider mb-1.5">Interest Rate (%)</label>
              ...
              <div>
                <label className="block text-xs text-on-surface/40 font-body uppercase tracking-wider mb-1.5">Tenure (months)</label>
              ...
              <div>
                <label className="block text-xs text-on-surface/40 font-body uppercase tracking-wider mb-1.5">Start Date</label>
                <input type="date" value={form.startDate} onChange={e => setForm({ ...form, startDate: e.target.value })} className={inputCls('startDate')} />
              </div>
              <div>
                <label className="block text-xs text-on-surface/40 font-body uppercase tracking-wider mb-1.5">End Date</label>
                <input type="date" value={form.endDate} onChange={e => setForm({ ...form, endDate: e.target.value })} className={inputCls('endDate')} />
              </div>
              <div className="sm:col-span-2 flex gap-3 pt-2">
                <button type="submit" className="bg-[#2962FF] hover:bg-[#2255DD] text-on-surface text-sm font-semibold px-5 py-2.5 rounded-xl transition-colors">
        ...
        {debts.length > 0 && (
          <>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="bg-on-surface/5 backdrop-blur-xl border border-on-surface/10 shadow-[0_4px_20px_rgba(0,0,0,0.2)] rounded-xl p-5">
                <p className="text-xs text-on-surface/40 font-body">Total Outstanding</p>
                <p className="text-xl font-bold text-[#EF4444] mt-1">{inr(totalOutstanding)}</p>
              </div>
              <div className="bg-on-surface/5 backdrop-blur-xl border border-on-surface/10 shadow-[0_4px_20px_rgba(0,0,0,0.2)] rounded-xl p-5">
                <p className="text-xs text-on-surface/40 font-body">Total Monthly EMI</p>
                <p className="text-xl font-bold font-headline text-on-surface tracking-tight mt-1">{inr(totalEMI)}</p>
              </div>
              <div className="bg-on-surface/5 backdrop-blur-xl border border-on-surface/10 shadow-[0_4px_20px_rgba(0,0,0,0.2)] rounded-xl p-5">
                <p className="text-xs text-on-surface/40 font-body">DTI Ratio</p>
                ...
                <p className="text-xs text-on-surface/40 font-body mt-1">{dtiPct > 40 ? 'High risk' : dtiPct > 25 ? 'Moderate' : 'Healthy'}</p>
              </div>
            </div>
            ...
            {debts.map(loan => {
              ...
              return (
                <div key={loan.id} className="bg-on-surface/5 backdrop-blur-xl border border-on-surface/10 shadow-[0_4px_20px_rgba(0,0,0,0.2)] rounded-xl p-5">
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      <h3 className="font-bold font-headline text-on-surface tracking-tight">{loan.name}</h3>
                      <p className="text-xs text-on-surface/40 font-body">{loan.bank} · {loan.interestRate}% p.a.</p>
                    </div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-5">
                    <div><p className="text-xs text-on-surface/40 font-body">Outstanding</p><p className="font-bold text-[#EF4444]">{inr(loan.outstanding)}</p></div>
                    <div><p className="text-xs text-on-surface/40 font-body">Principal</p><p className="font-bold font-headline text-on-surface tracking-tight">{inr(loan.principal)}</p></div>
                    <div><p className="text-xs text-on-surface/40 font-body">EMI</p><p className="font-bold font-headline text-on-surface tracking-tight">{inr(loan.emi)}</p></div>
                    <div><p className="text-xs text-on-surface/40 font-body">Rate</p><p className="font-bold text-[#2962FF]">{loan.interestRate}%</p></div>
                  </div>
                  <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                    <div>
                      <h4 className="text-sm font-bold text-on-surface/70 mb-3">Loan Progress</h4>
                      <DonutChart
                        segments={[
                    ...
                    {monthlyIncome > 0 && (
                      <div>
                        <h4 className="text-sm font-bold text-on-surface/70 mb-3">Debt Burden (DTI)</h4>
                        <DonutChart
                          segments={[
        ...
        {debts.length === 0 && !showForm && (
          <div className="bg-on-surface/5 backdrop-blur-xl border border-on-surface/10 shadow-[0_4px_20px_rgba(0,0,0,0.2)] rounded-xl p-12 text-center">
            ...
            <p className="text-on-surface/60 font-body mt-3 font-medium">No loans added yet</p>
            <p className="text-sm text-on-surface/40 font-body mt-1">Click "Add Loan" to track your debt.</p>
          </div>
        )}
      </div>
  );
